...
- Модуль = отдельный .py файл.
- Имя модуля всегда в snake_case.
- Имя модуля никогда не совпадает с именем класса.
- Модуль не должен быть в CamelCase.
- Модуль не должен называться как сущность из ТЗ (например, Book → book).
- Каждый класс должен находиться в своём модуле.
- Один модуль = один класс (если ТЗ не требует иначе).
- main всегда есть, но генерируется позже.
- depends_on — список модулей, от которых зависит текущий.
- Не больше N модулей.
- Верни ТОЛЬКО JSON.
...
Важно:
- Структура ответа приведена как пример. Не должны быть модули data, logic, если они не нужны.
- main всегда есть но генерируется позже.
- depends_on — список имён модулей, от которых зависит текущий.
- Не больше {self.MAX_MODULES} модулей.
- Верни ТОЛЬКО JSON.
"""
